...
> [!NOTE]
> Most/all of the recent changes are automated AI slop. I set up an autonomous loop running Qwen3.8-27b to automatically attempt to decompile functions. Before committing it's required to verify that the resulting compilation matches the original game binary byte-for-byte. The code it produces varies greatly in quality, but is rarely better than mediocre. I slopped a project explorer overview for real-time stats where you can explore the project status at [chat.bordplate.no](https://chat.bordplate.no). 
