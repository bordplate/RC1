...
// tracks levelCamData.pCollMoby's oClass/pos.z when it is set. The boot-ELF
// entry point is the unmangled Splat placeholder, so pin it with a symbol
// override rather than the cfront-mangled Camera_updateCollState__Fv.
void Camera_updateCollState(void) asm("func_001ED470");

void Camera_updateCollState(void) {
    Vec4 a;
    register CamCollState* ccs asm("$21") = &camCollState;
    register u8* levelAnchor asm("$22") = (u8*)&levelCamData.inner.dir290;
    CameraQuad* p70 = &ccs->v70;
    asm volatile("" : "+r"(p70));
    CameraQuad* p60 = &ccs->v60;
    asm volatile("" : "+r"(p60));

    FastVecNormalize(&a, (float*)levelAnchor, -1.0f);

    ccs->aPrev = ccs->aCur;
    *(CameraQuad*)&ccs->aCur = *(CameraQuad*)&a;

    register Vec4* p20 asm("$16") = &ccs->dir20;
    asm volatile("" : "+r"(p20));
    float d = FastVecDot(p20, &a);
    if (d < -0.98f) {
        a.x += 0.2f;
        a.y += 0.2f;
        a.z += 0.2f;
    ...
    ccs->dir20.x =
        Cam_InterpValues(ccs->dir20.x, a.x, &ccs->off50, 0.015f, 0.2f, 0.0f);
    ccs->dir20.y =
        Cam_InterpValues(ccs->dir20.y, a.y, &ccs->off54, 0.015f, 0.2f, 0.0f);
    ccs->dir20.z =
        Cam_InterpValues(ccs->dir20.z, a.z, &ccs->off58, 0.015f, 0.2f, 0.0f);
    FastVecNormalize(p20, p20, 1.0f);

    register u8* levelCursor asm("$20") = levelAnchor - 0x210;
    FastVecSub(p70, (float*)levelCursor, p60);
    FastVecLength(p70);
    d = FastVecDot(p70, &a);
    ccs->fA0 = d;
    CameraQuad t;
    ccs->fA8 = d;
    FastVecNormalize(&t, &a, d);
    *(CameraQuad*)&ccs->b = *(CameraQuad*)&t;
    FastVecSub(&ccs->v80, p70, &t);
    float len = FastVecLength(&ccs->v80);
    ccs->fA4 = len;
    FastVecScale(&ccs->v80, &ccs->v80, 1.0f / len);
    *p60 = *(CameraQuad*)levelCursor;

    levelCursor = levelAnchor - 0x290;
    LevelCamData* lcd = (LevelCamData*)levelCursor;
    if (lcd->inner.i2284 != 0x50 || lcd->inner.i2084 == 0x11) {
        ccs->f00 = lcd->inner.dir80[0];
        ccs->f04 = lcd->inner.dir80[1];
        ccs->f08 = Cam_InterpValues(ccs->f08, lcd->inner.dir80[2],
                                    &ccs->off10, 0.0075f, 0.175f, 0.0f);
        ccs->f0C = lcd->inner.dir80[2];
    } else {
        ccs->f00 = lcd->inner.dir80[0];
        ccs->f04 = lcd->inner.dir80[1];
    }

    register float* ringBase asm("$6") = ccs->ring;
    float* dst = ringBase;
    int i = 0;
    do {
        *dst = dst[1];
    ...
    ringBase[i] = lcd->inner.f98;

    register MobyInstance* mp asm("$4") = (MobyInstance*)lcd->inner.pCollMoby;
    u32 p = (u32)mp;
    if (p == 0 || mp->oClass == 0x4BA || mp->oClass == 0x336) {
        ccs->mobyZDelta = 0;
        ccs->pCollMoby = 0;
        ccs->lastMobyZ = lcd->inner.dir80[2];
    } else if (ccs->pCollMoby == p) {
        ccs->mobyZDelta = FastAbsF(mp->pos.z - ccs->lastMobyZ);
        if (ccs->mobyZDelta >= 0.001f)
            ccs->mobyZDelta = 0;
        ccs->lastMobyZ = ((MobyInstance*)ccs->pCollMoby)->pos.z;
    } else {
        ccs->mobyZDelta = 0;
        ccs->pCollMoby = p;
        ccs->lastMobyZ = mp->pos.z;
    }
}
