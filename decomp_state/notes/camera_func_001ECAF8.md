...
- When the occlusion-staged flag is 0 (`D_0018C318+0x14` == occlCamStaged,
  0x18C32C), commits the result: 128-bit copy `activeCam0 -> Camera`
  (0x187080 == &currentCamera.pos) and
...
- Advances the two clamped progress scalars:
  `field_1C = min(1, field_1C + posInterp * D_0015ED60)`,
  `field_10 = min(1, field_10 + quatInterp * D_0015ED60)`.
- Returns 0.
...
2. **scale pinned to `$f0`.** `register f32 scale asm("$f0") = D_0015ED60;`
   is required so the clamp FPU math uses scale=$f0, product=$f1, sum=$f2.
   Without it the clamp allocates wrong FPU regs (8-word diff).
...
4. **`CamTransitionWork` aggregate** (CameraQuad quat @ sp+0x00; CameraMatrix
   ...
   sp+0x10).
5. **`D_00187290` declared as a 64-byte CameraMatrix** (not u32) to produce
   the split-address RTL for the `func_001FA2B8` call.
6. **128-bit copy to `Camera`** (0x187080) uses pinned registers:
   `dst`->`$3`, `src`->`$4`, `value`->`$2`, with a tied `"+r"` barrier on
   dst/src.
7. **`D_0018C318` base (0x18C318) is loaded into s3 and +0x14 read** for both
   flag checks; the flag is occlCamStaged (0x18C32C) but the original does NOT
   address it directly — it keeps the base in s3. Using occlCamStaged directly
   changes the codegen.
...
## Placeholder names still to resolve (see refactor.json)

- `D_0015ED60` (transition step scale, f32).
- `D_0018C318` (base whose +0x14 is occlCamStaged).
- `D_00187290` (64-byte orientation matrix).
- `camPos16 asm("Camera")` — data symbol 0x187080 (== &currentCamera.pos)
  collides with struct Camera; rename the data symbol.
- callees func_002133D0 / func_002144D8 / func_001FA400 / func_001FA4F8 /
  func_001FA2B8 (separate INCLUDE_ASM, rename when decompiled).
